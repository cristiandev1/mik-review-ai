...
export class DeepSeekProvider implements AIProvider {
    ...
    async reviewCode(params: ReviewParams): Promise<ReviewResult> {
        ...
        const systemPrompt = [
            ...
            '4. **CRITICAL - CLEAR REPLACEMENT INSTRUCTIONS:** For every issue, provide a CONCRETE suggestion with:',
            ...
            '   ```',
            '   ```',
            '5. **CRITICAL:** Always show the EXACT code that needs to be replaced (copy from the diff), so the user knows precisely what to change.',
            '6. Keep suggestions concise - show only the line(s) that need to change.',
