...
async function run() {
    try {
        // 1. Get Inputs
        const provider = (core.getInput('provider') || process.env.AI_PROVIDER || 'openai').toLowerCase();
        ...
        if (provider === 'gemini') {
            ...
            apiKey = geminiApiKey || '';
            if (!apiKey) throw new Error('Gemini API Key is required when provider is set to gemini');
            if (!modelName) modelName = 'gemini-2.0-flash'; 
        } else {
            selectedProvider = AIProviderType.OPENAI;
            apiKey = openaiApiKey || '';
            if (!apiKey) throw new Error('OpenAI API Key is required when provider is set to openai');
            if (!modelName) modelName = 'gpt-4o';
        }

        // 2. Initialize Services
        const aiProvider = createAIProvider(selectedProvider, apiKey);
        ...
        // Basic check to avoid sending empty diffs or massive ones
        ...
        // 5. Generate Review
        core.info(`Generating review using ${selectedProvider} model: ${modelName}...`);
        ...
        const commentBody = `## 🤖 AI Code Review\n\n${reviewResult.review}\n\n---\n*Generated by mik-review-ai using ${selectedProvider}*`;
